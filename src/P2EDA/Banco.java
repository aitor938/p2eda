...
class CCCId implements Comparable<CCCId>{
	private String ccc;
	...
	public int compareTo(CCCId cccId){
...
public class Banco {
	...
	private static ColeccionAbb<CCCId, BankIn> arbolBB
	 = new ColeccionAbb<CCCId, BankIn>();
	...
	private static String modificarCuenta(String line){
		...
		if (contadorDatos == 0){
			...
			if (arbolBB.esta(id)){
				BankIn in = arbolBB.obtenerInformacion(id);
	...
	private static String eliminarCuenta(String line){
		...
		if (contadorDatos == 0){
			...
			if (arbolBB.esta(id)){
				output = ": " + id.toString() + "; "
				  + arbolBB.obtenerInformacion(id).toString() + "\n";
				arbolBB.borrar(id);
			}else{
				output = " DESECHADA: " + id.toString() + "\n";
			}
